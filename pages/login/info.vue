<template>
  <div class="flexCol gap-4 items-center ">
    <div class='flex gap-4 w-full'>
      <TheInput  className="bg-secondary-800" :validation="validation && !firstName" placeholder="مثلا سارا" label="نام" class="w-full" v-model="firstName"/>
      <TheInput className="bg-secondary-800" :validation="validation && !lastName" placeholder="مثلا راد" label="نام خانوادگی" class="w-full" v-model="lastName"/>
    </div>
    <TheChipsInput className="bg-secondary-800" class="w-full" label="موتور ها" placeholder="مثلا بنلی 250" v-model="motors"/>
      <TheInput className="bg-secondary-800" :validation="!email && validation " placeholder="sample@gmail.com" label="ایمیل" class="w-full" v-model="email"/>
...
const submit = async () => {
  ...
  try {
    const result = await putProfile({firstName:firstName.value,lastName:lastName.value,motors:motors.value},user.value?.profile?.id)
    if (result ) {
      if(email.value){
